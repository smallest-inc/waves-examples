...
NGROK_PUBLIC_URL = os.environ.get("NGROK_URL")  # Replace with your actual Ngrok URL
...
def get_ivr_client():
    ...
    ivr_client = plivo.RestClient(
        auth_id=os.environ.get(
            "AUTH_ID"
        ),  # Fetch Plivo Auth ID from environment variables
        auth_token=os.environ.get(
            "AUTH_TOKEN"
        ),  # Fetch Plivo Auth Token from environment variables
    )
    return ivr_client
...
call_payload = {
    "to": os.environ.get("TO_NUMBER"),  # Recipient phone number
    "from": os.environ.get("FROM_NUMBER"),  # Caller phone number
}
